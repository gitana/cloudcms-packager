...
module.exports.create = function(config, callback) {
    ...
    Gitana.connect(function(err) {
        ...
        // create a packager
        var packager = createPackager(".", workingFolder, outputPath, this, archiveGroup, archiveName, archiveVersion);
...
var createPackager = function(basePath, workingFolder, outputPath, gitana, archiveGroup, archiveName, archiveVersion)
{
    ...
    var _addDirectory = function(directoryName, callback)
    {
        ...
        var files = wrench.readdirSyncRecursive(directoryPath);
        for (var i = 0; i < files.length; i++)
        {
            ...
            if (filename.indexOf(".json") > -1)
            {
                ...
                if (type === "node" || type === "association")
                {
                    ...
                    // if the node has translations, add thos ein
    ...
    var _acquireGUIDsFromServer = function(idRequestCount, callback) {
        ...
        if (idRequestCount > 0)
        {
            report("Requesting " + idRequestCount + " IDs from the server");

            var i = 0;
            do
            {
                var chunkSize = 90000;
                if (i + chunkSize >= idRequestCount) {
                    chunkSize = idRequestCount - i;
                }

                report("Requesting chunk: " + i + " to " + (i + chunkSize));

                var headers = {};
                headers["Authorization"] = gitana.getDriver().getHttpHeaders()["Authorization"];
                headers["Content-type"] = "application/json; charset=utf-8";
                var URL = gitana.getDriver().baseURL + "/tools/generateids?count=" + chunkSize;
                ...
                {
                    var theseRecordIds = JSON.parse(response.getBody())["ids"];
                    for (var x = 0; x < theseRecordIds.length; x++)
                    {
                        recordIds.push(theseRecordIds[x]);
                    }
                }
                else
                {
                    callback({
                        "err": "HTTP call failed, " + response.getBody()
                    });
                    return;
                }
            ...
            while (i < idRequestCount);
        }

        report("request complete");

        callback(null, recordIds);
    };
    ...
    var _bindAttachmentsToObjects = function(callback)
    {
        ...
        compiler.eachRecord(function(alias, record) {
            ...
            for (var j = 0; j < record.attachments.length; j++)
            {
                ...
                var mimeType = mime.lookup(attachmentFilePath);
    ...
    var _verifyObjects = function(callback)
    {
        ...
        compiler.eachRecord(function(_doc, record) {
            ...
            if (record.path)
            {
                ...
                {
                    console.log("Object claims a path that already exists, object: " + JSON.stringify(object, null, true) + " collides with: " + JSON.stringify(objectsByPath[record.path], null, true));
                    err = true;
                }
    ...
    var _commitToDisk = function(archiveRootFilePath, archiveGroup, archiveName, archiveVersion, traverser, callback)
    {
        ...
        manifest["includes"] = [];
        manifest["tipChangesetOnly"] = true; // this is basically extra information indicating that everything was compressed to a single changeset
        manifest["sources"] = [];
        manifest["contents"] = {};
        ...
        platformObject["key"] = getPlatformId();
        ...
        repositoryObject["key"] = getRepositoryId();
        ...
        changesetObject["key"] = getChangesetId();
        ...
        traverser.eachRecord(function(_doc, record, c, max) {
            ...
            dependencyObject["typeId"] = "node";
            dependencyObject["id"] = record.id;
            dependencyObject["key"] = getChangesetId() + "_" + record.id;

            changesetObject["dependencies"].push(dependencyObject);
        });
    ...
    r.getNodeWithType = function(type)
    {
        var results = {};
    ...
    r.package = function(config, callback)
    {
        ...
        _createContainerPaths(function (err) {
            ...
            _parseReferences(function(err) {
                ...
                _acquireGUIDsFromServer(compiler.count(), function (err, recordIds) {
